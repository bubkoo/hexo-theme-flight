...
exports.list_archives = function(options) {
    ...
    var item = function(href, name, length) {
        if (style === 'list') {
            arr.push('<li>' +
                '<a href="' + root + archiveDir + '/' + href + '">' + name + '</a>' +
                (showCount ? '<span> (' + length + ') </span>' : '') +
                '</li>')
        } else {
    ...
    for (var i = oldest.year(); i <= newest.year(); i++) {
        ...
        if (type === 'yearly') {
            item(i, moment({
                y: i
            }).format(format), yearly.length);

            continue;
        }

        for (var j = 1; j <= 12; j++) {
            var monthly = yearly.find({
                date: {
                    $year: i,
                    $month: j
                }
            ...
            item(i + '/' + (j < 10 ? '0' + j : j), moment({
                year: i,
                month: j - 1
            }).format(format), monthly.length);
        }
    }
