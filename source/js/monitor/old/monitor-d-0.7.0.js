!window.monitor || (function (window, undefined) {
    ...
    function log(line, source, msg, code) {
        ...
        if (M.debug && window.console && console.log) {
            window.console.log("DOMLint: line:" + line + ", code: " + code + ", message:" + msg + ", source:" + source + "")
        }
    }
    ...
    function getFormName(node) { // 获取元素所在的表单
        if (!node || !node.form) {
            return '';
        }
        var frm = node.form;
    ...
    M.DOMLint = function (doc) {
        ...
        t = new Date() - t;
        if (M.debug && window.console && console.log) {
            console.log('Validate DOM Time: ', t, 'ms');
        }
        return {
            res: {
                css: res.css,
...
!window.monitor || (function (window) {
    var M = window.monitor,
        doc = window.document;
    window.setTimeout(function () {
        try {
            if ('function' === typeof(M.DOMLint)) {
                var lintResult = M.DOMLint(doc);
                M.report({
                    pageRes: {
                        res: lintResult.res,
                        htmlSize: lintResult.htmlSize
                    }
                });
                if (lintResult.htmlErr && lintResult.htmlErr.length > 0) {
                    M.report({
                        htmlError: lintResult.htmlErr
                    })
                }
            }
        } catch (ex) {
